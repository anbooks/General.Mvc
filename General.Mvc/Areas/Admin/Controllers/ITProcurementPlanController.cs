...
namespace General.Mvc.Areas.Admin.Controllers
{
    ...
    public class ITProcurementPlanController : AdminPermissionController
    {
        ...
        [HttpPost]
        [Route("importexcelPlancc", Name = "importexcelPlancc")]
        [Function("采购计划明细重传", false, FatherResource = "General.Mvc.Areas.Admin.Controllers.ITProcurementPlanController.ITProcurementPlanMainIndex")]
        public ActionResult Importcc(IFormFile excelfile, string returnUrl = null)
        {
            string s;
            Request.Cookies.TryGetValue("Plan", out s);
            int ida = Convert.ToInt32(s);
            ...
            if (submit == "计划明细重传")
            {
                ...
                try
                {
                    ...
                    using (ExcelPackage package = new ExcelPackage(file))
                    {
                        ...
                        var plan = _sysProcurementPlanService.getAll(ida);
                        foreach (var u in plan)
                        {
                            var odera = _sysProcurementPlanService.getById(u.Id);
                            odera.IsDeleted = true;
                            _sysProcurementPlanService.updateProcurementPlan(odera);
                        }

                        var main = _sysProcurementPlanMainService.getById(ida);
                        for (int row = 2; row <= rowCount; row++)
                        {
                            ...
                            try
                            {
                                ...
                                model.PlanItem = main.PlanItem;
                                model.Prepare = main.Prepare;
